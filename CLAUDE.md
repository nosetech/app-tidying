...
`position`/`size` はウィンドウ設定ごとに丸ごと省略できます。省略した場合、そのプロパティは
変更されず、アプリの起動・活性化のみが行われます。

```json
{ "app": "Finder" }
```

さらに、`position`/`size` オブジェクト内の `x`/`y`、`width`/`height` は**片方だけ**指定する
こともできます。指定していない側は変更されず、現在のウィンドウの位置・サイズが維持されます。
...
上記の例では、幅のみ画面の半分に変更され、高さは現在のウィンドウの高さのまま維持されます。

**注意**: 現在のウィンドウ位置・サイズが取得できない場合（新規ウィンドウ作成直後で
取得に失敗した場合等）は、未指定側はディスプレイ原点相当の値（`0`）にフォールバックします。

##### 4. タイリング指定（version 2.0、Issue #121 / #123）

`position`/`size` によるピクセル・パターン指定の代わりに、`tiling` フィールドで
OS標準タイリング相当のキーワードを直接指定できます（layout.json `version: "2.0"` が必要）。
      ...
      "displays": [
        {
          "name": "Built-in",
          "windows": [
            { "app": "Google Chrome", "tiling": "left" },
            { "app": "Slack", "tiling": "top-right" },
            { "app": "Zoom", "tiling": "full-screen" },
            {
              "app": "Safari",
              "position": { "x": 0, "y": 0 },
...
上記のように、同じ layout.json（version 2.0）の中でウィンドウごとに `tiling` 方式・
`position`/`size` 方式・どちらも指定しない方式を混在させて構いません。
...
| `bottom-left` | 左下4分割 |
| `bottom-right` | 右下4分割 |
| `full-screen` | 画面全体に表示（macOS標準のフルスクリーン機能。新しいSpaceへ切り替わる点に注意） |

**命名規則**:

- 英語表記・小文字・ケバブケース（`top-left` 等）で固定します。`position`/`size` の
  パターン指定（`left`/`right`/`half`/`third`/`max` 等）と表記を揃えることで一貫性を
  優先しています
- エイリアス（例: `fullscreen`）や大文字表記（例: `Left`）は許容しません。上記一覧に
  ない値が指定された場合はバリデーションエラーとなります（例:
  `無効な tiling 値: 'fullscreen' (left, right, top, bottom, top-left,
  top-right, bottom-left, bottom-right, full-screen を指定)`）

**`position`/`size` との相互排他制約**:
...
- ここでの「指定」は JSON上に `position`/`size` キー自体が存在するかどうかで
  ...
  対象になります
- `tiling`・`position`・`size` のいずれも指定しない場合は、既存仕様（任意指定、
  Issue #120）通りアプリの起動・活性化のみを行い、位置・サイズは変更しません
- `tiling` を指定した場合は、OS標準タイリング機能（「ウインドウ」メニュー操作）を
  使用して配置します
- `tiling` を指定せず `position`/`size` のみを指定した場合は、パターン指定・絶対座標
  指定を問わず常に直接プロパティ設定処理で配置します（OS標準タイリング機能は
  使用しません。詳細は次項「OS標準タイリング機能」を参照）

**ディスプレイ移動との関係**:

- `tiling` を指定したウィンドウは、まず所属する `displays[].name` が示す
  ディスプレイへメニュー操作（「\[ディスプレイ名\]に移動」）で移動してから、
  `tiling` に対応するタイル配置（「移動とサイズ変更」または「画面全体に表示」）を
  行います（ディスプレイ移動が先）
- ウィンドウが既に対象ディスプレイ上にある場合、ディスプレイ移動のメニュー項目は
  表示されないため、この移動操作の失敗は異常とはみなさず無視します（対象
  ディスプレイへの移動自体は不要なため）。それ以外の理由（「ウインドウ」メニュー
  自体が見つからない、Accessibility APIの権限がない等）による失敗は想定外の異常
  であるため区別し、WARNレベルでログ出力します（いずれの場合も、後続のタイル配置
  操作は変わらず実行します）

**メニュー操作が失敗した場合の扱い**:

- `tiling` 指定時は `position`/`size` によるフォールバック（直接プロパティ設定）が
  存在しないため、タイル配置のメニュー操作自体が失敗した場合（メニュー項目が
  見つからない、権限がない等）は、そのウィンドウの配置を失敗として扱います
- 複数アプリの操作の一部が失敗する状況のため、既存の「エラーハンドリング方針」
  （部分的な失敗への対応）に従い、他のウィンドウの処理は継続し、WARN レベルで
  通知します

**version 1.0 との互換性**:

- `tiling` フィールドは version 2.0 でのみサポートします。version 1.0 の layout.json
  で `tiling` が指定されていた場合はバリデーションエラーとなります（例:
  `version 1.0 では 'tiling' フィールドはサポートされていません
  （version 2.0 を指定してください）`）
- version 1.0 → 2.0 への移行は自動変換されません。既存の `position`/`size` 指定は
  version 2.0 でもそのまま使用できるため、`version` の値を `"2.0"` に変更するだけで
  移行できます（`tiling` フィールドを使わない場合）

#### OS標準タイリング機能（Issue #118 / #123）

macOS標準の「ウインドウ」メニュー（「移動とサイズ変更」「画面全体に表示」
「[ディスプレイ名]に移動」）操作を使ったウィンドウ配置機能です。

`position`/`size` プロパティを指定した場合は、パターン指定（`left`/`right`/`half`/`max` 等）
であっても**OS標準タイリング機能は使用せず、常に従来の直接プロパティ設定処理**で配置します
（Issue #123 の設計変更）。`position`/`size` からパターンを自動推測してOS標準タイリング
機能を優先使用していた旧仕様（Issue #118）は廃止されました。

OS標準タイリング機能は、layout.json version 2.0 の明示的な `tiling` フィールド
（仕様は前項「4. タイリング指定」、Issue #121で設計・Issue #122で実装）
を指定した場合にのみ動作します。

**注意**: 「画面全体に表示」（フルスクリーン相当）は、macOS標準のフルスクリーン機能であり、
**新しい仮想デスクトップ（Space）へ切り替わります**。単にウィンドウを画面いっぱいに
拡大するだけの動作ではない点に注意してください（実機検証で確認済み。技術的な検証の詳細は
`technical-verification/README.md` を参照）。

### フォーマットバージョニング

- JSON ファイル内に `version` フィールドを含める
- 現在サポートバージョン: `1.0`, `2.0`（`2.0` は `tiling` フィールドに対応。
  仕様は Issue #121 で確定、実装は Issue #122 で完了済み）
- 扱えないバージョンの場合はエラー表示
- 古いバージョンからの移行は README で説明（アプリケーション側では自動変換しない）
- `1.0` → `2.0` への移行は、`tiling` フィールドを使わない場合は `version` の値を
  変更するだけで完了する（`position`/`size` 指定は `2.0` でもそのまま使用可能）
...
定義ファイル（layout.json）で指定されているディスプレイが接続されていない場合、以下の動作を行います：

**フォールバック機能（推奨）**:
- 接続されている別のディスプレイでアプリケーションを起動
- 複数のディスプレイが接続されている場合は、1 番目のディスプレイを使用
- WARN レベルで「ディスプレイが接続されていない」ことを通知
- INFO レベルで「フォールバック先のディスプレイを使用する」ことをログ出力

**ログ出力例**:
```
[WARN] ディスプレイ 'SDM27U9M2*30' が接続されていません: 指定されたディスプレイ 'SDM27U9M2*30' が見つかりません
[INFO] ディスプレイ '内蔵Retinaディスプレイ' を使用してウィンドウを配置します（フォールバック）
```
...
#### 複数ウィンドウの扱い（1アプリケーション = 1ウィンドウ）

同一のアプリケーションで複数ウィンドウが開いている場合、`save` コマンドは各アプリケーションから最初のウィンドウのみを保存します：

- 保存対象: 最初に見つかったウィンドウ（1個のみ）
- スキップ対象: 同一アプリケーションの2番目以降のウィンドウ
- ログ出力: スキップされたウィンドウについてデバッグログに記録
...
この制限は、v1.0 の設計仕様です。ウィンドウは識別タイトルフィールドなしで管理されるため、個別のウィンドウを特定できません。
