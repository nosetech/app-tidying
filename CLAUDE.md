...
#### OS標準タイリング機能の優先使用（Issue #118）

位置指定が `left`/`right`/`top`/`bottom`、サイズ指定が `half`（4分割含む）、または
幅・高さともに `max` の組み合わせの場合、`position`/`size` プロパティの直接設定より前に、
macOS標準の「ウインドウ」メニュー（「移動とサイズ変更」「画面全体に表示」
「[ディスプレイ名]に移動」）操作を優先的に試みます。メニュー操作が失敗した場合は、
自動的に既存の直接プロパティ設定処理へフォールバックします。

**注意**: 幅・高さともに `max` を指定した場合に使用される「画面全体に表示」は、
macOS標準のフルスクリーン機能であり、**新しい仮想デスクトップ（Space）へ切り替わります**。
単にウィンドウを画面いっぱいに拡大するだけの動作ではない点に注意してください
（実機検証で確認済み。技術的な検証の詳細は `technical-verification/README.md` を参照）。

対象とならない組み合わせ（`third` 指定、絶対座標指定等）は、従来どおり
`position`/`size` プロパティの直接設定方式で処理されます。
