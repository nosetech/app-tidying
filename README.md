...
最新リリースのビルド済みバイナリをダウンロードしてください：

**[v1.1.0 リリースページ](https://github.com/nosetech/app-tidying/releases/tag/v1.1.0)**

バイナリをダウンロード後、以下のコマンドで配置してください：

```bash
sudo mv apptidying /usr/local/bin/
sudo chmod +x /usr/local/bin/apptidying
```
...
Automator や Quick Actions から apptidying を実行する場合は、追加の設定が必要です。

#### Accessibility 権限の付与

**Automator でアプリケーションを作成した場合**は、以下の両方に権限を付与してください：

1. **Automator.app に権限を付与**
   - システム設定を開く：Apple メニュー → システム設定 → プライバシーとセキュリティ → Accessibility
   - ロック マークをクリックして変更を許可
   - 「+」ボタンをクリック
   - `/Applications/Automator.app` を選択して追加

2. **作成したアプリケーションに権限を付与**
   - 同じく Accessibility セクションで「+」ボタンをクリック
   - `~/Applications/` または `~/Desktop/` に保存されている Automator で作成したアプリケーションを選択して追加

#### ⚠️ 重要: バイナリ更新時の注意

apptidying のバイナリをアップデートした場合、**Accessibility 権限を付け直す必要があります**。

macOS のセキュリティ機構により、バイナリが更新されるとコード署名が変更され、権限情報がリセットされます。以下の手順で権限を再付与してください：

1. システム設定 → プライバシーとセキュリティ → Accessibility を開く
2. **Automator で作成したアプリケーション**を削除（「-」ボタンをクリック）
3. 再度そのアプリケーションを追加（「+」ボタンをクリック）

**注**: Automator.app 自体の権限は通常リセットされませんが、念のため確認してください。

権限が正しく再設定されるまで、作成したアプリケーションから apptidying を実行するとアクセシビリティ権限のエラーが発生します。
...
### Q. レイアウトで指定したディスプレイが接続されていない場合はどうなりますか？

**A.** App Tidying はディスプレイフォールバック機能により、接続されている別のディスプレイでアプリケーションを起動します：

**動作**:

- 指定したディスプレイが見つからない場合、接続されているディスプレイの最初のものを自動選択
- 複数ディスプレイがない場合（ディスプレイが全く接続されていない）は、エラーで処理を中止
...
これにより、ディスプレイの着脱が多い環境でも、作成したレイアウト定義が無効化されずに利用できます。
