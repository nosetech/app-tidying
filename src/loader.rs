...
fn process_window(
    ...
    // 4. OS標準メニュー操作を優先的に試行する（位置・サイズのいずれかが指定されている場合のみ）
    //    成功した場合は以降の直接プロパティ設定処理をスキップする。失敗しても
    //    既存の直接プロパティ設定処理へそのままフォールスルーするため、ここでのエラーは
    //    致命的として扱わない（Issue #118）。
    if window_config.position.is_some() || window_config.size.is_some() {
        // 4-1. ディスプレイ移動を試行する
